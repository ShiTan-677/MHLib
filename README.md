...
## 项目实现原理
本项目通过复刻预约网页源代码，复刻预约页面。但是由于项目所有者没有能力在普通浏览器导入微信的登录信息，且没有能力使用微信浏览器复刻网页，因此选择通过修改图片的方式伪造凭证。此后将伪造凭证上传至[图床](https://imgloc.com/ "某免费图片外链网站")，并将预约页面部分图片、图标超链接至伪造凭证以实现项目预期效果。
